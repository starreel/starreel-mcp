...
export function registerProduceTools(server: McpServer, client: StarReelClient) {
  ...
  server.tool(
    'apply_recovered_final',
    '把某个**历史终拼**设为本集当前成片(免费,指针切换)。'
    ...
    async ({ episode_id, merge_id }) =>
      jsonResult(await client.producePost(`/episodes/${episode_id}/asset-recovery/finals/${merge_id}/apply`, {})),
  )
  server.tool(
    ...
    async ({ episode_id, generation_id, target_storyboard_id }) =>
      jsonResult(await client.producePost(
        `/episodes/${episode_id}/asset-recovery/candidates/${generation_id}/apply`,
        { target_storyboard_id },
      )),
  )
  server.tool(
    ...
    async ({ episode_id, event_id }) =>
      jsonResult(await client.producePost(
        `/episodes/${episode_id}/asset-recovery/events/${event_id}/rollback`, {})),
  )
  server.tool(
    'list_wardrobe',
    '列本剧的**服装资产库**(免费)。每件服装是一个可被镜头引用的结构化资产,'
      + '不是分镜里的文字描述。',
    { drama_id: z.number().int().positive() },
    async ({ drama_id }) => jsonResult(await client.produceGet(`/dramas/${drama_id}/wardrobe`)),
  )
  server.tool(
    'create_wardrobe',
    '新建一件服装资产(免费,纯文本写库)。建好之后要**绑到镜头上**才会影响出图——'
  ...
  server.tool(
    ...
    async ({ wardrobe_id }) => jsonResult(await client.produceGet(`/wardrobe/${wardrobe_id}`)),
  )
  server.tool(
    'update_wardrobe',
    '改服装资产(免费)。改完**不会**自动重出已生成的图:要让画面跟上得再调 regenerate_wardrobe_image,'
      + '已出的镜头帧还要逐镜重出。',
    {
      wardrobe_id: z.number().int().positive(),
    ...
    async ({ wardrobe_id, ...fields }) =>
      jsonResult(await client.producePut(`/wardrobe/${wardrobe_id}`, fields)),
  )
  server.tool(
    'regenerate_wardrobe_image',
    '重出这件服装的资产图(**收费**,图片步按用量后付)。改完 description/参考图之后用它让图跟上。',
    { wardrobe_id: z.number().int().positive() },
    async ({ wardrobe_id }) =>
      jsonResult(await client.producePost(`/wardrobe/${wardrobe_id}/regenerate`, {})),
  )
  server.tool(
    'delete_wardrobe',
    '软删一件服装资产(免费)。已绑定它的镜头不会被自动改,先确认没有镜头还在用。',
    { wardrobe_id: z.number().int().positive() },
    async ({ wardrobe_id }) => jsonResult(await client.produceDelete(`/wardrobe/${wardrobe_id}`)),
  )
  server.tool(
    'build_wardrobe_timeline',
    '算**服装时间线**:从剧本与分镜推出每个角色在各镜应该穿哪一身,并给出逐镜建议 diff。'
    ...
    { episode_id: z.number().int().positive() },
    async ({ episode_id }) =>
      jsonResult(await client.producePost(`/episodes/${episode_id}/wardrobe-timeline`, {})),
  )
  server.tool(
    'apply_wardrobe_timeline',
    '把客户勾选的换装建议落库(第②步,免费)。改的是每镜的 **active_wardrobe_id 结构化绑定**,'
    ...
    async ({ episode_id, items }) =>
      jsonResult(await client.producePost(`/episodes/${episode_id}/wardrobe-timeline/apply`, { items })),
  )
  server.tool(
    'apply_wardrobe_appearances',
    '逐角色回填 appearances 的服装绑定(第②步的多角色版,免费)。'
    ...
    async ({ episode_id, items }) =>
      jsonResult(await client.producePost(
        `/episodes/${episode_id}/wardrobe-timeline/apply-appearances`, { items })),
  )
  server.tool(
    'get_world_state',
    '读本集的**世界状态账本**：被追踪的实体(道具/服装/伤痕一类)在各镜之间的状态，'
      ...
      + '\n逐条事件的文字详情在官网面板看，这里给结构化概览。',
    { episode_id: z.number().int().positive() },
    async ({ episode_id }) => jsonResult(await client.produceGet(`/episodes/${episode_id}/world-state`)),
  )
  server.tool(
    'rebuild_world_state',
    '重建世界状态账本(**要花钱**：走 LLM 从分镜里抽取实体与状态变化)。'
      + '\n改完分镜、或 get_world_state 回 stale=true 时才需要重建;没改过就别重复跑。',
    { episode_id: z.number().int().positive() },
    async ({ episode_id }) =>
      jsonResult(await client.producePost(`/episodes/${episode_id}/world-state/rebuild`, {})),
  )
  server.tool(
    'add_world_state_event',
    '往账本里加一条**手工事件**(免费)。用途：AI 没抽到、或抽错了的状态变化，由你补一条说明。'
    ...
    async ({ episode_id, ...body }) =>
      jsonResult(await client.producePost(`/episodes/${episode_id}/world-state/manual`, body)),
  )
  server.tool(
    'delete_world_state_event',
    '删掉一条手工事件(免费)。只能删手工加的,AI 抽取出来的要靠 rebuild_world_state 重算。',
    {
      episode_id: z.number().int().positive(),
      event_id: z.number().int().positive(),
    },
    async ({ episode_id, event_id }) =>
      jsonResult(await client.produceDelete(`/episodes/${episode_id}/world-state/manual/${event_id}`)),
  )
  server.tool(
    'link_world_state_scene_variant',
    '把账本里的某条事件关联到一个**场景变体**(免费)。'
    ...
    async ({ episode_id, event_id, variant_id }) =>
      jsonResult(await client.producePut(
        `/episodes/${episode_id}/world-state/events/${event_id}/scene-variant`,
        { variant_id },
      )),
  )
  server.tool(
    'derive_pose_chain',
    '派生**姿态链**(Action Graph)：从相邻镜的动作推出每镜的起止姿态，让镜间衔接不跳。'
      + '**要花钱**(走 LLM)。'
      + '\n★只填**空着**的那些镜，已经写了姿态的不动;黑场镜自动跳过。'
      + '\n出图前做，能减少「上一镜站着下一镜突然坐着」这类接不上的情况。',
    { episode_id: z.number().int().positive() },
    async ({ episode_id }) =>
      jsonResult(await client.producePost(`/episodes/${episode_id}/derive-pose-chain`, {})),
  )
  server.tool(
    'quote_extend_final',
    '★成片续写**报价**(免费·无副作用):在已有成片后面再接几轮 AI 生成的片段,把片子拉长。'
  ...
  server.tool(
    'extend_final',
    '发起成片续写(**收费**,每轮走视频生成扣费链)。后台串行跑,立即返回会话 id,用 get_extend_final 轮询。'
    ...
    async ({ episode_id, ...body }) =>
      jsonResult(await client.producePost(`/episodes/${episode_id}/extend-final`, body)),
  )
  server.tool(
    'get_extend_final',
    '查本集所有续写会话的进度与产物。免费。'
      ...
      + '\n★status=processing 时耐心轮询,别重复发起 extend_final(那会再扣一次钱)。',
    { episode_id: z.number().int().positive() },
    async ({ episode_id }) => jsonResult(await client.produceGet(`/episodes/${episode_id}/extend-final`)),
  )
  server.tool(
    'apply_extend_final',
    '把某个续写产物**设为本集成片**(免费,纯指针切换)。'
    ...
    async ({ episode_id, session_id }) =>
      jsonResult(await client.producePost(`/episodes/${episode_id}/extend-final/${session_id}/apply`, {})),
  )
  server.tool(
    'export_sheet_compare',
    '导出**设定图对照版**(角色设定图与成片画面的对照拼版,营销/交付用)。触发后台合成,用 get_sheet_compare 轮询。'
    ...
    async ({ episode_id, character_id }) =>
      jsonResult(await client.producePost(
        `/episodes/${episode_id}/export/sheet-compare`,
        character_id ? { character_id } : {},
      )),
  )
  server.tool(
    'get_sheet_compare',
    '轮询设定图对照版的导出状态。免费。回 enabled(剧集开关是否打开)/processing/result/error。'
      + '\n★enabled=false 就是那个剧集开关没开——不是出错,去官网剧集设置里打开再导。',
    { episode_id: z.number().int().positive() },
    async ({ episode_id }) =>
      jsonResult(await client.produceGet(`/episodes/${episode_id}/export/sheet-compare`)),
  )
